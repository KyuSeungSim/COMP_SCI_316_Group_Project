...
public class FractalController {
   ...
   private void drawFractal() {
      ...
      drawFractal(level, 40.0, 40.0, 350.0, 350.0);
   }
   ...
   public void drawFractal(int level, double xA, double yA, double xB, double yB) {
	...
	// base case: draw a line connecting two given points
	      if (level == 0) {
	         ...
	         // isosceles right triangle between (xA, yA) and (xC, yC) 
	         // where the right angle is at (xD, yD)
	         double xD = xA + (xC - xA) / 2 - (yC - yA) / 2;
	         double yD = yA + (yC - yA) / 2 + (xC - xA) / 2;
	         
	         double xE = xC + (xD-xC) * Math.cos(angle) + (yD-yC) * Math.sin(angle);
	         double yE = yC - (xD-xC) * Math.sin(angle) + (yD-yC) * Math.cos(angle);

	         drawFractal(level - 1, xE, yE, xA, yA);
	         drawFractal(level - 1, xE, yE, xC, yC);
	         drawFractal(level - 1, xE, yE, xB, yB);  
	      }
